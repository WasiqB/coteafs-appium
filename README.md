...
## Logging:
Customizable logging of low level events is done by using [coteafs-logger](https://github.com/WasiqB/coteafs-logger). You can create your own logger config _(and you can also refer details on how to write it on the project README file)_. By default, it will search logger config path in system properties `coteafs.logger.config`. If the properties is not defined, than it will search for file named `coteafs-logger.yaml` in the resource folder. If that is also not defined, it will use it's default setting and will create log files as mentioned below.
